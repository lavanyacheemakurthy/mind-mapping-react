        ...
        <div className={css.container}>
            ...
            <div>
                        ...
                        return (
                            <ChartElement
                                ...
                                pathsForPlayAnimation={
                                    props.runAnimation && determinePaths(elements) && determinePaths(elements).paths ? determinePaths(elements).paths : []
                                }
                            />
                        );
                    })}
                ...
                {props.runAnimation && (
                    <div>
                        <div>
                            {determinePaths(elements) && determinePaths(elements).paths
                                && determinePaths(elements).paths.length > 0 && determinePaths(elements).paths?.map((x, i) => {
                                    return (
                                        <div>
                                            <b style={{ color: "magenta" }}>
                                                Path for {i + 1} ) {x[x.length - 1].name} is :{" "} </b>
                                            <span style={{ color: 'blue' }}>{x
                                                .slice(1)
                                                .map((p) => {
                                                    if (p.displayShape === SHAPES.ARROW) {
                                                        return "Inputs as : " + p.inputsList.map(x => x.key + " : " + x.value).join(" || ")
                                                    } else {
                                                        return p.name
                                                    }
                                                })
                        ...
                        <div>
                            ...
                            <ul>
                                {determinePaths(elements) && determinePaths(elements).testCases
                                    && determinePaths(elements).testCases.length > 0 && determinePaths(elements).testCases?.map((x, i) => {
                                        return (<li type="square" style={{ color: 'green' }}>
                                            {x}
                                        </li>)
                                    })}
                            </ul>
